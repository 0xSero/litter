...
The helper also points every mission at one Cargo target cache under the repository's Git common directory. The coordinator permits at most one Cargo-capable mission at a time, giving incremental artifacts a single writer; non-Rust missions may still run concurrently. Rust missions apply the repository's pinned Codex patch set once before their first Cargo command, keep it applied for the entire validation pass, and restore the submodule only after the final Rust command; patched and unpatched dependency graphs must never alternate in the shared cache. Implementation missions use focused test filters while iterating. Every validation command captures complete output in a mission-local log while preserving the command's exit status; Pi inspects that log after failure and never reruns unchanged compilation merely to recover output hidden by filters or terminal truncation. Expensive full crate, platform, and workspace suites run once on the combined wave validator, except where a brief explicitly names one final pre-integration gate. A timed-out full suite is recorded and returned to Codex; Pi must not blindly restart it. Codex does not duplicate an unchanged full suite merely to re-prove Pi's valid event-log evidence.
...
|---|---|---|---|---|---|---|---|
| 00 | `pi-chat-w00-m00-reconcile` | COMPLETE | ACCEPTED | `codex/pi-chat-w00-m00-reconcile` | `019ff2c5-ca5b-7659-9c70-c62941da9a94` | `70def155` | `work/chat-performance/tasks/task-00.md` |
