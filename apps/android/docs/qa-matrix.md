...
| SSH changed identity (guided connect) | Both platforms route a changed host key during guided connect to the confirm modal instead of raw error text; Replace syncs saved servers first so mid-session discoveries can be re-pinned. Device acceptance pending. |
