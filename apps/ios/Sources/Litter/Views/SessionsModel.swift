...
final class SessionsModel {
    ...
    private struct DerivationInputs: Equatable {
        ...
        let selectedServerFilterId: String?
        let showOnlyForks: Bool
        let selectedRuntimeKind: AgentRuntimeKind?
        let workspaceSortMode: WorkspaceSortMode
        let searchQuery: String
        let frozenMostRecentOrder: [ThreadKey]?
    }

    // Transcript events change the global snapshot without changing sessions.
    // Comparing the actual inputs avoids rebuilding the complete session tree.
    @ObservationIgnored private var cachedDerivationInputs: DerivationInputs?
    ...
    private func refreshState() {
        ...
        let snapshot = withObservationTracking {
            ...
            let derivationInputs = DerivationInputs(
            ...
            if cachedDerivationInputs == derivationInputs, let cached = cachedDerivedData {
                ...
                cachedDerivationInputs = derivationInputs
