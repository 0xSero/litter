...
Fix every finding in this same mission and worktree. Re-read the task acceptance criteria, inspect the current diff and commits, and rerun only validation invalidated by the repair. Use the shared CARGO_TARGET_DIR=$cargo_target_dir and the narrowest relevant test filters; the coordinator permits only one Cargo-capable mission at a time so incremental artifacts have a single writer. Before any Cargo command, apply the pinned dependency patch set once with ./apps/ios/scripts/sync-codex.sh --preserve-current; keep it applied through all Rust validation and restore the submodule only after the last Cargo command. Never compile the unpatched submodule between patched validations. Do not repeat an unchanged full suite that already produced valid evidence, and never automatically retry a timed-out full suite. Do not push or open a PR. If a finding is factually wrong, respond with concrete source or command evidence instead of silently ignoring it.
